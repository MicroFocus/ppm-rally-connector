...
public class RallyTimeSheetIntegration extends TimeSheetIntegration {
    ...
    public List<Field> getMappingConfigurationFields(ValueSet paramValueSet) {
        return Arrays.asList(new Field[] {
                ...
                new CheckBox(Constants.KEY_REMOVE_ITEMS, "IS_REMOVE_ITEMS_WITHOUT_TIMELOG", true), new LineBreaker()});
    }
        ...
        for (final Workspace workspace : workspaces) {
            ...
            for (final Project project : projects) {
                ...
                if (values.get(Constants.KEY_DATA_DETAIL_LEVEL).equals(Constants.KEY_DATA_DETAIL_LEVEL_USERSTORY)) {
                    // work_items
                    List<HierarchicalRequirement> backlogs = rallyClient.getHierarchicalRequirements();
                    ...
                    for (HierarchicalRequirement backlog : backlogs) {
                        ...
                        items.add(new RallyExternalWorkItem(project.getName(), backlog.getName(), hms, values,
                                startDate, endDate));
                    }
                } else {
                    // iteration
                    ...
                    for (final Iteration iteration : iterations) {
                        ...
                        items.add(new RallyExternalWorkItem(project.getName(), iteration.getName(), hms, values,
                                startDate, endDate));
                    }
                }
            }
